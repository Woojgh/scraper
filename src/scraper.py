...
INSPECTION_DOMAIN = 'https://eutils.ncbi.nlm.nih.gov/entrez/eutils/'
INSPECTION_PATH = 'epost.fcgi?db=$db&id=$id_list'
INSPECTION_PARAMS = {
    'Output': 'W',
    'Business_Name': '',
    'Business_Address': '',
...
wiki = 'http://info.kingcounty.gov/health/ehs/foodsafety/inspections/Results.aspx'
...
def get_inspection_page(**kwargs):
    ...
    html_file = open("inspection_page.html", "w")
    html_file.write(str(resp.content))
    html_file.close()
...
def load_inspection_page():
    ...
    params = INSPECTION_PARAMS.copy()
    for key, val in load_inspection_page.items():
        if key in INSPECTION_PARAMS:
            params[key] = val
    resp = requests.get(url, params=params)
    print(url)
